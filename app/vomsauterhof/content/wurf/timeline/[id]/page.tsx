...
function Page() {
  ...
  const fetchWurf = async () => {
    try {
      ...
      // For non-timeline categories, fetch old single-entry timeline data
      if (wurfData.wurf.category === "wurf-a") {
        const timelineResponse = await fetch(
          `/api/wurf/timeline/get?wurfId=${wurfId}`
        );
        const timelineData = await timelineResponse.json();
        if (timelineData.success) {
          setInformation(timelineData.timeline.information || "");
        }
      }
    } catch (error) {
  ...
  const fetchTimeline = async () => {
    try {
      const response = await fetch(`/api/wurf/timeline/list?wurfId=${wurfId}`);
  ...
  const handleSubmitInformation = async () => {
    try {
      const response = await fetch("/api/wurf/timeline/update", {
        method: "PUT",
        headers: {
  ...
  const handleDelete = async (ids: string[]) => {
    ...
    try {
      const response = await fetch("/api/wurf/timeline/delete", {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
  ...
  const toggleSelectAll = () => {
    if (selectedIds.size === timeline.length) {
      setSelectedIds(new Set());
    } else {
      setSelectedIds(new Set(timeline.map((t) => t.id)));
    }
  };
  ...
  const isAllSelected =
    selectedIds.size === timeline.length && timeline.length > 0;
            ...
            <AnimatePresence mode="popLayout">
              ...
              >
                ...
                >
                  ...
                  <span className=" font-medium opacity-50">Hunde</span>
